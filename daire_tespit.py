...
configfile = 'daire_tespit_config.csv'
...
while 1:
    ...
    #-------------------------------
        ...
        for c in contours:
            ...
            # plot
            radius = int((x2 - x1) / 2)  # calculate the radius of the circle
            center_x = int((x1 + x2) / 2)  # calculate the x-coordinate of the circle center
            center_y = int((y1 + y2) / 2)  # calculate the y-coordinate of the circle center
            cv2.circle(frame0, (center_x, center_y), radius, (0, 0, 255), 2)  # draw the circle
            #cv2.putText(frame0, f'{radius:.2f}', (center_x, center_y - radius - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

          
            # add dimensions
            draw.add_text(frame0, f'{xlen/2:.2f}', x1-((x1-x2)/2), min(y1,y2)-8, center=True, color='red')
